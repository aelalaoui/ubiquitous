  ...
  while (Date.now() - startTime < config.tx.get_retry_timeout) {
    try {
            ...
            if (response.data && response.data.length > 0) {
                ...
                const instruction = instructions.find((ix) => ix.programId === config.liquidity_pool.radium_program_id);
  ...
  try {
    ...
    JSON.stringify({
            ...
            priorityLevelWithMaxLamports: {
                maxLamports: 1000000,
                priorityLevel: "veryHigh", // If you want to land transaction fast, set this to use `veryHigh`. You will pay more.
            },
            },
        }),
        {
